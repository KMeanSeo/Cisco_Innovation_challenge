...
const ADMIN_EMAIL = "admin@sample.wbx.ai";
  ...
  try {
    ...
    await fetch("https://webexapis.com/v1/messages", {
      ...
      body: JSON.stringify({
        ...
        files: [
          `https://noble-tammara-kicksco-97f46231.koyeb.app/uploads/20250409_reAdjustPM.jpg`,
        ],
      }),
    });
...
app.post("/api/pm-adjusted", upload.single("image"), async (req, res) => {
  ...
  try {
    ...
    res.status(200).json({
      message: "조정 요청이 전송되었습니다. 관리자의 승인을 기다리는 중입니다.",
    });
  } catch (err) {
...
app.post("/webhook", async (req, res) => {
  try {
    ...
    // 2) 관리자가 "승인" 또는 "거부" 명령을 입력한 경우 (관리자만)
      ...
      (msgText.startsWith("승인") || msgText.startsWith("거부")) &&
      ...
      if (targetEmail && pendingRequests[targetEmail]) {
        if (msgText.startsWith("승인")) {
          ...
          const formattedDate = `${now.getFullYear()}년 ${(
            "0" +
            (now.getMonth() + 1)
          ).slice(-2)}월 ${("0" + now.getDate()).slice(-2)}일 ${(
            "0" + now.getHours()
